...
// ... (tipi omessi per brevità, tieni quelli che avevi)
type AppUser = { id: number; nome: string; cognome: string; email: string; };
type Message = { toUserId?: number; from: "admin" | "user"; text: string; date: string; userId?: number; };
...
export default function Utenti() {
  ...
  useEffect(() => {
    setUsers(JSON.parse(localStorage.getItem("utenti") || "[]"));
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter(u => u.nome.toLowerCase().includes(q) || u.cognome.toLowerCase().includes(q) || u.email.toLowerCase().includes(q));
  }, [search, users]);

  const sendMessage = (userId: number) => {
    const text = prompt("Scrivi il messaggio per l'utente:");
    if (!text) return;
    const newMessages = [...messages, { toUserId: userId, from: "admin" as const, text, date: new Date().toISOString() }];
  ...
  const readMessages = (userId: number) => {
    ...
    alert(userMessages.map((m) => ` ✉ ${m.text}`).join("\n"));
  };

  const hasMessages = (userId: number) => messages.some((m) => m.from === "user" && m.userId === userId);

  return (
    <div className="page-wrapper max-w-5xl animate-fade-in">
      <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
        <button onClick={() => router.back()} className="btn-ghost rounded-full p-2 w-10 h-10 flex-shrink-0">
      ...
      <div className="mb-6 sm:mb-8 relative max-w-md w-full">
        ...
        <input type="text" placeholder="Cerca utente..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-12 rounded-full" />
      </div>
            ...
            <div key={u.id} className="bg-[var(--color-surface)] p-5 rounded-2xl border border-[var(--color-border)] shadow-sm flex flex-col gap-4">
              ...
              {role === "admin" && (
                <div className="flex gap-2 mt-2 pt-4 border-t border-[var(--color-border)]">
                  ...
                  <button onClick={() => readMessages(u.id)} className="flex-1 py-3 text-sm btn-ghost relative">
                    Leggi
