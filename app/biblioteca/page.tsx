  ...
  const libriFiltrati = useMemo(() => {
    ...
    return libriTotali.filter((libro) => {
      const matchSearch =
        ...
        libro.fraseFamosa?.toLowerCase().includes(query) ||
        libro.genere?.toLowerCase().includes(query);

      // ✅ SUPPORTO GENERI MULTIPLI
      ...
      const matchGenere =
        genere === "tutti" ||
        generiLibro.includes(genere.toLowerCase().trim());

      return matchSearch && matchGenere;
    });
  }, [search, libriTotali, genere]);

  return (
    <div className="page-wrapper animate-fade-in">
      <h1 className="text-3xl sm:text-4xl font-bold tracking-tight mb-6 sm:mb-8 text-[var(--color-text-primary)]">
        La Collezione
      </h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          ...
          {role === "admin" && (
            <>
              ...
              <button
                className="btn-ghost w-full sm:w-auto py-3 sm:py-2"
